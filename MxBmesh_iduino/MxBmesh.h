...
private:
	...
	////
	static byte sequenceid;
	static uint16_t destaddress;
	static byte srcaddress_last;
	static byte destaddress_last;
	static uint16_t srcaddress;
	static byte messagetype;
	static byte nodeindex; //my node index in the path
	static byte pathcount; //my node to sink path count
	static byte pdataindex;
	static byte pathindex;
public:
	MXBMESH();
